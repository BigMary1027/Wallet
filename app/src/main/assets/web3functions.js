...
const contractAddress = '0xB617D1AC68C184d193a79cd5C8FF55a69A4f08e6';
...
const updateBalance = async () => {
	try {
			...
			document.getElementById('balanceDisplay').innerText = `余额: ${web3.utils.fromWei(balance, 'ether')} eCNY`;
	} catch (error) {
...
const updateTransactions = async () => {
	try {
			// 使用新的getTransactionsForAccount方法
			...
			transactionsDisplay.innerHTML = ''; // 清空旧数据

			transactions.forEach(transaction => {
					const amountEth = web3.utils.fromWei(transaction.amountETH.toString(), 'ether'); // 支出
					const amountECNY = web3.utils.fromWei(transaction.amountECNY.toString(), 'ether'); // 获得
...
const handleTransaction = async (method, args) => {
    try {
        ...
        alert(`${method}失败: ${error.message}`); // 用户反馈
    }
};
...
//         const mnemonic = bip39.generateMnemonic(); // 使用bip39库生成助记词
//         document.getElementById("mnemonicDisplay").innerText = mnemonic; // 显示助记词
//     });
// });
